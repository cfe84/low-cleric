...
describe("Schedule", () => {
  ...
  const schedule = calculator.calculateSchedule([leadTime1, leadTime2, leadTime3, leadTime4, leadTime5],
    BracketUtils.createDateBracket(new Date(2020, 3, 6, 1), 0));

  // then
  it("skips weekends", () => {
    should(schedule[0].batch).eql(batch1)
    should(schedule[0].leadTime).eql(leadTime1)
    should(schedule[0].scheduledStartDate.calculated).eql(new Date(2020, 3, 6, 1))
    should(schedule[0].scheduledFinishDate.calculated).eql(new Date(2020, 3, 13, 1))
    should(schedule[1].batch).eql(batch2)
    should(schedule[1].leadTime).eql(leadTime2)
    should(schedule[1].scheduledStartDate.calculated).eql(new Date(2020, 3, 13, 1))
    should(schedule[1].scheduledFinishDate.calculated).eql(new Date(2020, 3, 20, 1))
    should(schedule[2].batch).eql(batch3)
    should(schedule[2].leadTime).eql(leadTime3)
    should(schedule[2].scheduledStartDate.calculated).eql(new Date(2020, 3, 20, 1))
    should(schedule[2].scheduledFinishDate.calculated).eql(new Date(2020, 3, 22, 1))
  })

  it("skips holidays", () => {
    should(schedule[3].batch).eql(batch4)
    should(schedule[3].leadTime).eql(leadTime4)
    should(schedule[3].scheduledStartDate.calculated).eql(new Date(2020, 3, 22, 1))
    should(schedule[3].scheduledFinishDate.calculated).eql(new Date(2020, 4, 5, 1))
  })

  it("rounds portions of days up", () => {
    should(schedule[4].batch).eql(batch5)
    should(schedule[4].leadTime).eql(leadTime5)
    should(schedule[4].scheduledStartDate.calculated).eql(new Date(2020, 4, 5, 1))
    should(schedule[4].scheduledFinishDate.calculated).eql(new Date(2020, 4, 6, 1))
  })
})